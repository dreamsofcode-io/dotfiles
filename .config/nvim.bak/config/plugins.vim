...
" NERDTree on when open
autocmd StdinReadPre * let s:std_in=1
autocmd VimEnter * if argc() == 0 && !exists("s:std_in") | NERDTreeToggle | endif

let NERDTreeIgnore=['\.o$', '\~$', '^moc_.*\.cpp$', '\.a$', '\.lo$', '^CMakeCache.txt$', '^cmake_install.cmake$', '^CMakeFiles$']
...
autocmd BufWritePre *.{cpp,h,c,hs,yaml,scss,vue,js} silent! %s/\s\+$//
...
let g:LanguageClient_serverCommands = {
    ...
    \ }
    " 'java': ['/usr/local/bin/jdtls-lc', '-data', getcwd()],
...
set hidden
let g:racer_cmd = "/home/elliott/.cargo/bin/racer"
